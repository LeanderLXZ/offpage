...
class RunMetricsRecorder:
    ...
    def record(self, lane_name: str | None, result: "LLMResult") -> None:
    ...
    def summarize(self) -> None:
        """Print a per-phase / per-lane-type aggregate table."""
        with self._lock:
        ...
        print("=" * 60)
        header = (f"  {'phase':<10} {'lane':<16} {'calls':>5} "
                  f"{'dur_s':>8} {'in_tok':>10} {'out_tok':>9} {'cost':>8}")
        print(header)
        print("  " + "-" * (len(header) - 2))
        tot_calls = tot_in = tot_out = 0
        tot_dur = tot_cost = 0.0
        for (phase, lane), a in sorted(agg.items()):
            print(f"  {phase:<10} {lane:<16} {int(a['calls']):>5} "
                  f"{a['dur']:>8.1f} {int(a['in']):>10} {int(a['out']):>9} "
                  f"{a['cost']:>8.4f}")
            ...
            tot_cost += a["cost"]
        print("  " + "-" * (len(header) - 2))
        print(f"  {'TOTAL':<10} {'':<16} {tot_calls:>5} {tot_dur:>8.1f} "
              f"{tot_in:>10} {tot_out:>9} {tot_cost:>8.4f}")
        print("=" * 60)
...
def record(lane_name: str | None, result: "LLMResult") -> None:
    """Append one metrics row (no-op if uninitialised or on any error)."""
    if _recorder is None:
        return
    try:
        _recorder.record(lane_name, result)
    except Exception:  # noqa: BLE001
