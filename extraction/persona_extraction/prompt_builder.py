...
from .lifecycle.progress import StageEntry, PipelineProgress
...
def _render_lane_scope_block(lane_scope: str) -> tuple[str, str]:
    ...
    block = (
        ...
        "里出现。**本清单优先于下文对这些字段的任何『必填 / 均已填写』"
        "表述**——下文按单 lane 模式（你写整份快照）撰写，那时它们确实要你"
        "写；本次是 sub-lane 模式，写了就会触发上面说的 merge hard fail。\n\n"
