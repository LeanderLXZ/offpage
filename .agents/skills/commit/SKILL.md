...
下方流程分为 `## Step 0:` ~ `## Step 5:`（前置一段 `## $ARGUMENTS 解析` 是参数解析，不算正式 step）。每进入一个 step，**先打印一行进度**：`[/commit] Step N: <子段标题>`，让用户实时看到当前位置；最后一步执行结束打印 `[/commit] done`。跳过某 step 时打印 `[/commit] Step N: 跳过（理由：…）` 而不是静默略过。
