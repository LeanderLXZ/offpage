...
- **若完全没有改动**（working tree 干净 + index 空）：本轮当前分支已无新 commit 可造，**直接跳过 Step 2-4 进 Step 5**（forward/merge 阶段；当前分支可能仍领先其他本地分支，需要 sync 出去）。在 <进度工具> 里把 Step 2/3/4 一次性标 `completed`，对话里各打一行 `Step N 跳过（理由：working tree 干净，本轮无新 commit）`；Step 5 仍按 auto-sync vs 普通模式正常运行。若 Step 5 也判定无任何分支需要同步（所有候选分支都已 ancestor）→ 打印"无改动 + 所有分支已同步"收尾
