---
name: plan
description: 激活后强制锁定为只讨论模式——禁用 Write / Edit / NotebookEdit；Bash 仅允许只读查询命令（cat / grep / ls / find / git log / git diff / git status / wc / head / tail 等），任何 mutating 命令（git add / commit / push / pull / merge / checkout / reset / rm / mv / mkdir / touch / 写文件 / 网络写）禁止；不主动触发 /go / /commit / /todo-add / /post-check / /full-review 等写 skill；不开 plan.md / draft.md / notes.md 临时文件。讨论收敛后由用户自行调用写 skill 落盘。$ARGUMENTS = 本次讨论主题（可省）。用户说 '/plan'、'纯讨论'、'只讨论不动文件'、'分析一下方案'、'plan 一下'、'先讨论再说' 等触发。
---
