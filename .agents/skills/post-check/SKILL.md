...
> Step 2 / Step 3 跑完后，把双轨结论（计划项落实状态、Findings 列表、Missed Updates、Open Questions、Residual Risks）**先 hold 在脑里 / 笔记里**，不要立刻打印。Step 4 用结构化摘要回写 log + commit；Step 5 才把完整报告打印到对话——这样完整报告就是 `/post-check` 输出的最后一段，用户看完就能直接拍板，不需要往回翻屏。

## 4. 回写 log（摘要版）+ commit

把双轨结论的**结构化摘要**追加到 **intent 基线那份 log 文件**（Step 0.5 读的那份），**不重复贴完整 Findings 全文**（全文留给 Step 5 在对话里展开，避免 log 文件爆炸）：
...
log 缺失时：打印"⚠️ 无 log 可回写，复查结论仅在对话中保留"，并**直接进入 Step 5**（无 commit 可做）。
...
- **只读 + 单写单 commit 例外**：不改代码、不改 docs、不改 schema / prompt / ai_context；**唯一允许的写 + commit 是 Step 4 那份 log 摘要回写**——其他 dirty 文件一律不动、不提交
...
- **输出顺序硬约束**：log 回写 + commit (Step 4) **先于** 对话报告输出 (Step 5)。完整双轨报告必须是 `/post-check` 在对话里的**最后一段实质内容**——Step 6 只放一行"等待确认"短语作收尾，不要在报告之后再跟一段总结 / commit 提示 / next steps，否则用户又得回滚屏幕
