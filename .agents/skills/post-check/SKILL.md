...
- **不超出本次 intent 范围扩功能** — 只针对 Findings / Open Questions 给建议，不"顺手"建议重构无关代码
- **能 1 行修就别写 10 行** — 推荐保守路径而非"全部修"；可观察 / 可推迟 / 可降级的，明说"建议留作 todo"
- **Open Questions 给方向不替用户做选择** — 列 1-2 候选 + 推荐项 + 一句话理由，留拍板权给用户
- **诚实标注不确定** — 自己也拿不准的就说"建议先观察 / 等 runtime 数据"，不硬给结论

### 针对 Findings
- **{H1/M1/L1}** → 建议：{做 / 留 todo / 跳过} — {一句话理由}
- ...

### 针对 Open Questions
- **OQ1** {Open Question 摘要} → 候选 A：... / 候选 B：... → **推荐 A，理由：...**
- ...

### 总览
- 本轮**强烈建议立刻修**：{Hx, My, ...}（一行能修的小问题或会真实影响下一阶段的 bug）
- 本轮**建议留作下一轮 todo**：{...}（边界改进 / 防未来误改 / 需要更多数据）
- 本轮**建议跳过 / 关闭**：{...}（false positive 或与本次 intent 不相关）
```
