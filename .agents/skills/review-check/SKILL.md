---
name: review-check
description: 复核 review 报告 — 读取 docs/review_reports/ 下最近一份报告（可选按模型过滤：claude / codex / 具体 slug；不传参则取全局最新），逐条复核 finding / risk / open question 是否仍真实存在，给出证据（文件+行号）与落地方案草稿，输出不落盘不改代码，等用户确认后再用 /go 执行。用户说"复核 review"、"核一下 codex/claude 的 review"、"review-check" 时触发。
---
