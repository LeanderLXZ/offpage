...
## 0. 环境 & 自动锁定工作位置
项目硬规则：代码 / schema / prompt / docs / ai_context / skill 变更**先进 master**，其他分支通过 `git merge master` 前向同步。`/go` 的 git 交互契约：**Step 0 到 Step 9 中途一次都不问**，只在全部完成后按需问一次是否切回 master。
...
| 当前状态 | 自动动作 | 工作位置 |
|---|---|---|
| 已在 master 且工作区 clean | 原地编辑 | 主 checkout |
| 其它（非 master / dirty / 有运行进程） | `git worktree add ../<repo>-master master`；后续编辑、Step 1 PRE log、Step 8 commit 全部走这个 worktree | `../<repo>-master` worktree |

- 选定后打印一行策略声明：`策略：master 原地` 或 `策略：../<repo>-master worktree 隔离`。主 checkout 在 worktree 路径下**全程不被动**（运行中的进程、dirty 工作区都原样保留）
...
Step 0 已经把工作位置锁在 master（主 checkout 或 worktree），**不再询问**切 master。

- `git status` 只剩本次改动；扫禁提路径（`sources/` 原文、数据库、embeddings、caches、真实 user packages）
- message 风格对齐 `git log --oneline -10`；按逻辑单元分 commit
- 提交后 `git status` 确认干净
- **若 Step 0 走了 worktree 路径**：commit 完成后立刻回到主 checkout（无需离开主 checkout 的原分支）并 `git worktree remove --force ../<repo>-master`；commit 已落到 master ref，worktree 目录删除不丢数据

## 9. 同步其他分支 + 末尾单次询问
`git branch --format='%(refname:short)'` 列出所有本地分支；对每个非 master 分支判断是否已含本次 master 改动（`git merge-base --is-ancestor master <branch>` → 0 即已同步）。未同步的分支逐个处理，**中途不询问**：

- 若分支有正在运行的进程（如 `extraction/*`）或未完成工作 → 记录到 `docs/todo_list.md` 推迟，不强推
- 否则 merge 进去：
  - 若该分支 = 当前主 checkout 所在分支（走 worktree 路径时常见 = 原分支）→ 直接 `git merge master`，无需 checkout
  - 否则 → `git checkout <branch> && git merge master`
...
**此时——也只有此时——询问用户一次**：
- 若当前 HEAD == master → 无询问，直接结束
- 若当前 HEAD != master → 问一次："所有分支已同步，当前停在 `<branch>`，是否 `git checkout master`？"。同意则切；用户说停留则留在当前分支
