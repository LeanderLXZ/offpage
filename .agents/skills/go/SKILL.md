---
name: go
description: 方案落盘 — 按上文讨论把 schema/代码/config/docs 改动按 11 步流程（加载配置 → 环境锁定 → PRE log → requirements → 实现 → 测试 → 文档对齐 → 全库 review → POST log → commit → 分支同步）推进到可交付状态。Step 2 先把讨论/决策/计划/验证标准登记到 logs/change_logs/ 的 PRE 段，执行结束再回写 POST 段，作为 /post-check 的 intent 基线。用户说"落地"、"执行方案"、"go"、"把刚才讨论的改下来" 时触发。
---
...
## 4. 核心实现
按讨论改 schema、prompt template、架构代码、配置。对照 `ai_context/conventions.md` 的 Cross-File Alignment 表列出连带文件（该表不存在则跳过本项，仅按本次改动直觉判断）。

## 5. 轻量测试（仅有代码 / schema 变更时）
