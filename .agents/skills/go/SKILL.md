---
name: go
description: 方案落盘 — 按上文讨论把 schema/代码/config/docs 改动按 10 步流程（环境 → PRE log → requirements → 实现 → 测试 → 文档对齐 → 全库 review → POST log → commit → 分支同步）推进到可交付状态。第一步先把讨论/决策/计划/验证标准登记到 docs/logs/ 的 PRE 段，执行结束再回写 POST 段，作为 /post-check 的 intent 基线。用户说"落地"、"执行方案"、"go"、"把刚才讨论的改下来" 时触发。
---
