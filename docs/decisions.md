...
47. Phase 0 摘要子进程超时 = `[phase0].summarize_timeout_s`（默认 1800s），而不是与 phase 4 场景切分共用同一个 600s 预算。原因：一个 Phase 0 chunk 读 `chunk_size` 章（默认 20），并在 opus-4-7 effort=max 下产出 N× per-summary（100–150 字）+ 5 个 chunk 级二级聚合（`chunk_arc_summary` / `chunk_world_rules` / `chunk_power_levels` / `chunk_factions` / `chunk_regions`）；运行时证据表明 wall > 600s 属正常。phase 4 场景切分的 600s 预算（`[phase4].scene_split_timeout_s`，#68 起独立命名）不受影响，无撞线证据。→ `extraction/config.toml` `[phase0]`、`extraction/persona_extraction/core/config.py::Phase0Config`、`extraction/persona_extraction/orchestrator.py:_summarize_chunk`。本条只解耦 phase 0；repair 的 L3 语义审校由 #64 单独定预算。
...
63. **LLM 子进程起在独立进程组，超时杀整棵进程树（用 Ctrl+C 换不死锁）。**
    ...
    **曾考虑的保守版**（只给 kill 后的 `communicate()` 加 timeout，不动进程组）：
    ...
    → logs/change_logs/2026-07-16_112206_preflight-unattended-run.md。
64. **L3 语义审校超时 = `[repair].semantic_timeout_s`（默认 900s）—— 权威位置是 config，`repair/` 内不得硬编码。**
    ...
    **两层根因**。其一是**接线**：`checkers/semantic.py` 写死
    ...
    `persona_extraction.core.config`，因此值必须来自 config 而非代码字面量；
    ...
    **743s 从何而来**：本条初版取 1200 —— 当时全部数据在旧 600s 天花板处右删失
    ...
    值，900 是基于实测尾部的有据值。
    **边界**：本条只定 L3 语义审校（Phase A 检查 + gate 复检，共用
    `SemanticChecker._review_file`）的**取值** 900s。T1 / T2 / triage 各自的
    600 / 600 / 300 保持不变 —— 实测 T1 p50=14s / max=411s，无撞线证据；
    把这三个值同样收进 config、并统一四者的传递形态，由 #68 落地。
    Plumbing → `extraction/config.toml` `[repair]`、
    `extraction/persona_extraction/core/config.py::RepairAgentConfig`、
    `extraction/persona_extraction/orchestrator.py`（repair `_llm_call`）、
    `extraction/repair/checkers/semantic.py::_review_file`、
...
66. **L3 gate 复检改定点：per-file 只复检「本轮改过的 json_path ∪ 本轮携带的未修语义 issue path」（放弃每轮全文复检）。**
    ...
    **边界**：Phase C fallback L3（`gate_ever_ran==False` 的兜底）保持全文——它
    ...
    `_path_in_scope` + `protocol.BACKEND_FAILURE_RULES`）、
...
68. **repair 四类 LLM 调用的 timeout 全部由调用点显式传出，注入方不留兜底 default。**
    **背景**：#64 修掉了 `checkers/semantic.py` 的硬编码，但同一结构问题在
    `repair/` 内另外三处仍在：`fixers/local_patch.py` 传字面量 600、
    `fixers/source_patch.py` 传 600、`triage.py` 传 300，全都不读 config，
    违反 `conventions.md §Single Source of Truth`。#64 的教训是硬编码会
    shadow 掉 config、让整个配置层静默失效。
    **决策**：`RepairConfig` 携带四个 timeout 字段（`semantic_timeout_s` 900 /
    `t1_timeout_s` 600 / `t2_timeout_s` 600 / `triage_timeout_s` 300），由
    `orchestrator` 从 `[repair]` 填入；`coordinator` 透传给
    `SemanticChecker` / `LocalPatchFixer` / `SourcePatchFixer` / `Triager`；
    四个 `_llm_call` 调用点各传各的。`orchestrator` 两个包装器里的
    `default_timeout`（phase 3）/ `default_review_timeout`（phase 2）**删除**，
    ...
    **为何是「调用点传」而非「注入方持有」**：(1) 与 #65 的 effort 归属
    ...
    TypeError 而非静默走兜底 —— 正是 #64 诊断出的失效模式的结构性防御。
    `repair/` 仍然 config.toml-agnostic：它不读 toml，只消费注入方填好的
    `RepairConfig`（与既有 `max_rounds` / `accept_cap_per_file` /
    `retry_policy` 形态一致）。这是对 #64 **传递机制**的收紧，#64 的取值
    结论（900 = 实测尾部 743s 的 1.2×）与解耦结论均不变。
    **附带的命名归属清理**：`[phase3].review_timeout_s` 只剩两个引用点且都不
    属于 phase 3 —— 唯一真实消费者是 phase 4 scene split，另一个是 phase 2
    repair 注入点的死代码 default（#59 缩水版下 phase 2 唯一可达的 LLM 调用是
    T1，而它显式传值）。故重命名移段为 `[phase4].scene_split_timeout_s`；死
    代码随统一模型删除。
    **边界**：纯 refactor，四个值一个不动，行为不变（已逐调用点验证实际传出
    900 / 600 / 600 / 300 与改动前相等）。不给 phase 2 repair 接 L3（#59
    缩水版是有意设计）。`validate_only()` 也加 `config` 参数，避免公共 API 上
    出现「semantic 超时不可达」的死角。
    Plumbing → `extraction/config.toml`（`[repair]` 三个新键 + `[phase4]`
    移入键）、`extraction/persona_extraction/core/config.py`
    （`RepairAgentConfig` / `Phase3Config` / `Phase4Config`）、
    `extraction/repair/protocol.py::RepairConfig`、
    `extraction/repair/coordinator.py`（`_build_pipeline` / `_build_fixers` /
    `Triager` 构造点）、`extraction/repair/{checkers/semantic,fixers/local_patch,
    fixers/source_patch,triage}.py`、
    `extraction/persona_extraction/orchestrator.py`（两个 `_llm_call` +
    两个 `RepairConfig` 构造点）、
...
74. **Phase 3.5 结清回路：不自造判定 + 未结清一律回台账 + 两次机会后记录
    放弃。** #72 六段关卡首跑（53 stage / 33m11s / 159 次 LLM 调用）后残留
    8 条债，逐条追因发现三条是结构性死结、另有一处会在台账清空后变成假
    PASS。四处修法：

    **(a) 长度容忍度对齐。** 修复循环用 #48 容差门（`min/maxLength` ±10%，
    且必须整个错误集合都是长度类才放宽）判 `LENGTH_TOLERANCE_PASS`，而
    Phase 3.5 的 schema 复校走严格 `validate_only`。实测两条债正好卡在
    容忍带内（`maxLength 200` 的字段实为 208、`30` 的实为 33）：修复循环
    判 PASS 且不写 resolution，门判 unsettled，**再跑多少次都不会变**。
    长度容差判据提升为公开的 `length_tolerance_pass`，复校委托给它 ——
    权威只有一个：门不能比修复循环更严，否则容忍带就是死结带。判据里
    **必须**保留"整个错误集合都是长度类才放宽"这一半，且它被折进函数本体
    而非留给调用方：宽松化的只是 JSON Schema，而 structural / cross_file
    这些检查器根本不在 schema 里，少了这道守卫，一条 structural 债会被
    "宽松校验通过"洗成已结清。
    ...
    **(b) unverified 债独立成类。** `semantic_unavailable` /
    ...
    3.5 对该文件跑**不带 seed** 的完整 repair 事务 —— Phase A 的全文语义
    审校本身就是重新检查，查出的问题自带真实 `json_path` 并在同一事务里
    被修。不选「给它单独造一条复检路径」：那是把 Phase A 重写一遍。

    **(c) 未结清项一律回写台账。** 段 4 有 2 条审校发现修不掉，既不进台账
    也不进最终报告 —— 段 6 只裁决程序化层 + 台账债。本次因台账仍有 8 条而
    ...
    **(d) 两次机会 + 记录放弃。** 每文件最多 2 次事务：第一次终止于安全阀
    ...
    （残留是文件当前状态 + 上一刀失败在哪，信息量严格大于原始陈述）并改用
    `[llm].effort` 复读 —— 逐字重试只会复现同一终止条件。不设第三次：全价
    重推同一个终止条件，而扛过两次知情尝试的债需要的是人不是又一轮。仍
    失败写 `resolution="given_up"`（含 `attempts` / `last_error`），门里
    降为 warning、不阻断 Phase 4，但 verdict 单独成段列出（用户 2026-08-14
    拍板：放行但显著记录）。**`given_up` 与 `fixed` 的区别是承重的**：
    `fixed` 声称「已修好」，故只写给非可复验类（给 schema 债写 `fixed` 会
    永久压制它，即便字段后来又坏了）；`given_up` 对文件不作任何声称、只降
    严重度，故所有类别都能写 —— 文件真被修好时 schema 复校仍让债自然消失。
    **两类失败不算用尽机会**：事务抛异常，以及残留里含 `BACKEND_FAILURE_
    RULES` 的规则 —— 后者**不抛异常**（checker 有意把后端故障转成一条阻塞
    issue，好让它不被当成干净通过），事务因此正常返回 FAIL，与用尽在返回值
    上无法区分，只能靠规则名认出。二者都是基础设施故障，静默放弃会把一次
