...
**推迟原因**：本 commit 生成时，`extraction/<work_id>` 主 working tree 仍有 S003 未提交脏（baseline 6 份 M + 5 份未跟踪 `stage_snapshots/S003.json` / `memory_timeline/S003.json`），是上次 orchestrator checkout_master 失败留下的半成品，需要 `--resume` 继续处理，不能此刻切分支 merge。

**改动清单**：
- `git checkout extraction/<work_id>`
...
**验证**：
- `git log --oneline extraction/<work_id> | head -3` 看到 merge commit + `fc1faea`
