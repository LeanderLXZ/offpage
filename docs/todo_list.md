...
|---|---|---|---|---|---|
| `T-CONSISTENCY-TARGETS-SUBSET` | docstring/文档（schema description / validator.py:264 / consistency_checker.py:11-12 / decisions #13 / extraction_workflow / requirements §9.2）都承诺 stage_snapshot 三 map keys ⊆ baseline.targets cross-file hard fail，但代码层未实现——只跑了 _check_target_map_counts。2026-04-30 加了 maxItems=15 + 裁剪规则后该硬约束更关键。 | 🟢 High | ✅ Ready | 🟡 Medium | 无 |
| `T-MIGRATE-TARGET-BASELINE-ZH` | 2026-04-30 schema 改：relationship_type 英文 17-enum → 中文柔性 string 14 候选；tier 路人→普通。现有 phase 2 已 commit baseline 全是英文值，新 schema 校验 fail。需迁移脚本（含 17→14 中文映射 + tier 替换）。 | 🟢 High | ✅ Ready | 🟢 Small | 无 |
...
**Total**: 13 — 🟢 In Progress 2 ｜ 🟡 Next 5 ｜ ⚪ Discussing 6
...
### [T-CONSISTENCY-TARGETS-SUBSET] 补 cross-file ⊆ 校验：stage_snapshot 三 map keys ⊆ baseline.targets

**上下文**

`schemas/character/target_baseline.schema.json:5` 的 description、`automation/persona_extraction/validator.py:264` 的注释、`automation/persona_extraction/consistency_checker.py:11-12` 的模块 docstring、`ai_context/decisions.md` #13、`docs/architecture/extraction_workflow.md:113-118`、`docs/requirements.md` §9.2 都明文声明：phase 3 stage_snapshot 中 `target_voice_map` / `target_behavior_map` / `relationships` 的 keys MUST be ⊆ `target_baseline.targets[].target_character_id`，cross-file hard fail（无 escape hatch）。

但在代码层 grep 后发现该校验**没有真正实现**——consistency_checker.py 只跑 `_check_target_map_counts`（每 target 的 examples 计数），repair_agent 也没 cross-file ⊆ 检查。docstring/文档承诺与代码不符。

2026-04-30 改 `target_baseline.targets` 加 maxItems = 15（通过 `schemas/_shared/targets_cap.schema.json` $ref 共享继承）后，超限 → LLM 必须裁剪；裁剪策略要求 stage_snapshot 不写 baseline 没列的角色——但代码端没有该校验，只靠 prompt 文字软约束，run 时若 LLM 越界写入也不会 fail，会留下文档承诺与实际行为脱钩的硬伤。

**改动清单**

- `automation/persona_extraction/consistency_checker.py`：新增 `_check_target_keys_subset_baseline(stage_snapshot, baseline)`，对每个角色每个 stage 检查三 map 的 target_character_id（relationships 字段）/ target_type（voice_map / behavior_map 字段——非具体 character_id 时跳过，仅对具体名字判断 ⊆）⊆ baseline.targets[].target_character_id；错误项进 `consistency_report.json` 的 issues
- `automation/repair_agent/checkers/`：考虑加一个 cross-file checker 让 phase 3 内 repair lifecycle 也能捕获（不仅 phase 3.5 末端）；评估代价
- 跑现有 work 的 phase 3.5 用新校验扫一遍，看 S001 / S002 是否已经有 baseline 没列的 target 写入

**完成标准**

- 新校验跑一遍现有 work 的 baseline + stage_snapshot 不报漏检（或确认 prompt 软约束已经把 LLM 输出约束在 baseline 内）
- consistency_checker.py 模块 docstring 描述与代码一致
- decisions.md #13 / docs 中"cross-file hard fail"承诺与代码行为对齐

**依赖**：无（独立漏洞修复）

**触发**：2026-04-30 `target_baseline maxItems = 15 + tier 路人→普通 + relationship_type 中文化` /go 时识别（详见 [logs/change_logs/2026-04-30_014942_target_baseline_zh_and_cap.md](../logs/change_logs/2026-04-30_014942_target_baseline_zh_and_cap.md)）

---

### [T-MIGRATE-TARGET-BASELINE-ZH] 迁移现有 target_baseline.json：英文 enum → 中文柔性 string + tier 路人→普通

**上下文**

2026-04-30 schema 改：`target_baseline.relationship_type` 从英文 17-enum 改为中文柔性 string（14 候选），`tier` 从 `路人` 改 `普通`。当前已有 work_id 的 phase 2 产出 `works/<work_id>/characters/*/canon/target_baseline.json` 全是英文 enum 值 + 老 tier，新 schema 校验会全部 fail。

需要把现有 baseline 里的 17 个英文值映射到中文 14 候选 + 老 tier `路人` → `普通`。映射表：
- `close_kin` → `至亲`
- `lover` → `恋人`
- `close_friend` → `挚友`
- `mentor` → `师长`
- `disciple` → `弟子`
- `friend` → `朋友`
- `ally` / `colleague` / `subordinate` / `superior` → `同僚`（合并）
- `acquaintance` / `stranger` / `passerby` → `路人`（合并）
- `rival` → `对手`
- `enemy` → `敌人`
- `nemesis` → `敌人`（合并；如 description 里描述了"宿敌"特质，可在 description 字段保留更精细描述）
- `other` → 走 fallback：根据 description 选合适 14 候选或自定义中文短词

**改动清单**

- 写一个一次性迁移脚本 `scripts/migrate_target_baseline_zh.py` 或在 `automation/persona_extraction/migrate_*.py` 系列下加一个，按上表替换 enum + tier
- 跑现有 work_id 的 baseline 校验通过
- 评估：是否要重跑 phase 2 baseline（让 LLM 用新 prompt 重新选）反而更准——目前 phase 2 已 commit，重跑代价高，倾向脚本迁移

**完成标准**

- 当前 work 的 `target_baseline.json` jsonschema 校验通过（含 maxItems 15 + 新 tier enum）
- 迁移脚本可重跑（幂等）

**依赖**：无（脚本工具任务）

**触发**：2026-04-30 schema 改触发的存量数据迁移需求（详见同上 change log）
...
- 三方 keys（target_voice_map / target_behavior_map / relationships）⊆
  baseline.targets[].target_character_id（cross-file hard 校验，违规 fail
  而非 warning）
