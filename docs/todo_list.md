...
`ai_context/decisions.md` #15 明确 detail level 按 `tier` 分层（核心 / 重要 → ≥3–5 examples；次要 / 普通 → terse；从未登场 → empty entry）——bound 本就因 importance / tier 而异。一致性检查不区分 importance / tier，会对次要 / 普通 target 过度报错（field_completeness 把可选字段也按核心 target 标准查）或对核心 / 重要 target 过宽。
