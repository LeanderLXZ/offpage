...
**任务废弃**：写一条 `docs/logs/` 记录废弃原因后从本文件删除。

**讨论转落地**：讨论中章节的方案定案后，把它从"讨论中"移到"立即执行"或"下一步"，并把条目补全成完整任务（动机/清单/验证/依赖/完成标准）。
...
2. **StageEntry 增加 `failed_lanes` 字段**
   - 形如 `failed_lanes: [{"lane_type":"char_snapshot","char_id":"Character B","last_error":"exit 1: ...","failed_at":"2026-04-18T10:02:36Z"}]`
...
3. **`_extraction_output_exists` → `_missing_lanes`**
   ...
   - 现在返回 `bool`，改为返回 `list[LaneKey]`（`LaneKey` 可以是 `tuple[str, str]` 如 `("char_snapshot", "Character B")`）
...
1. **累计 token 使用到 work-local jsonl**
   ...
   - 每条记录一次成功的 claude -p 调用：
     ```json
     {"ts":"2026-04-18T10:02:36Z","stage_id":"阶段02_离开<stage_title>","lane":"char_snapshot:Character B","pid":179868,"duration_s":2325,"input_tokens":45823,"output_tokens":12045,"cache_read_tokens":...,"cache_write_tokens":...,"cost_usd":0.3421,"num_turns":8,"session_id":"..."}
     ```
