...
### [T-PHASE35-DEFERRED-FIX] Phase 3.5 重做为六段最终关卡 · 完成于 2026-08-10 · 完整完成

原为决策 #60 的 Part B（读台账逐条精修）。实测旧 Phase 3.5 后扩为整体重做
（决策 #72）：9 项检查里唯一报出东西的 alias 检查 40 条**全部误报**（两个集合
语义不同，前提不成立，删除）；38 条台账债一条不在检查面上且台账**零消费方**。
重做为三层（L1 结构 / L2 派生 1:1 / L3 台账结清）+ 六段（程序全扫 → 结清债 →
跨阶段审校 → 定点修 → 重投影 → 复扫门判）。L3 不采信台账陈述——schema 类
重校验自愈、semantic 类凭 append-only resolution 结清。新增全流水线唯一的
跨阶段并列视图（Phase A 一次只读一个文件），吃程序拼的瘦投影、按窗口切分。
coverage 账本使 `passed = error==0 && skipped==0`，静默跳过变显式失败。
**顺带定位并根治 length 债的制造机**（决策 #73）：17 条里 16 条是 repair 自己
造的——T2 改写散文后超限被判 `introduced` → 回归阀停轮 → 入台账；改为轮内
先 T0 消掉本轮自造的超限。附带 T0 截断改句读边界收口 + index-keyed dict → array。
实测：alias 误报归零、L3 读出全部 38 条债并按四条分支正确裁决、repair 两个
smoke 模块全过、投影 53 stage 切 9 窗。
