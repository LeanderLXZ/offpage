...
- 1 行摘要：5 项完成标准核查 4 ✅ + 2 ⚪。✅ 1 (4 件套 schema 删除 + `stage_snapshot.schema` 加 `failure_modes` 4 sub-class maxItems 15/15/10/10) / ✅ 3 (本轮 phase 2 跑通后 `works/<work_id>/characters/{Character A,Character B}/canon/` 均无 4 件套文件) / ✅ 4a (`automation/persona_extraction/prompt_builder.py:685-723` `_build_char_snapshot_read_list` 实际只读 stage_snapshot schema + identity.json + prev snapshot + source chapters，不含 4 件套 / character manifest) / ✅ 4b (本轮 phase 3 S001 抽取实测：Character B stage_snapshot.failure_modes 4 sub-class items 数 9/10/7/6，Character A 9/10/7/7，items 含完整字段 id/name/description/why_it_happens/correct_behavior/common_triggers) / ✅ 5 (ai_context/docs 同步早已落地)。⚪ 2 (legacy work migration) 转 N/A——本仓库无 legacy 4 件套 work，migration script 无适用对象。⚪ 4c (命中 maxItems 时裁剪生效) 未触顶——S001 items 6-10 低于 maxItems 10-15，本轮无机会触发；prompt 已含决策 #11e maxItems-aware truncation rule，后续 stage 内容密集时自然验证。**注**：本轮 phase 3 S001 整体 commit FAILED（因Character B `knowledge_scope.knows` + `relationships.target_known_status` 与 `stage_events` 事实矛盾，属决策 #11f prev_stage 四态规则 B 态执行不到位的单独问题），与本 todo 设计意图（4 件套废弃 + failure_modes inline 进 stage_snapshot）完全无关——failure_modes 字段在 FAIL 文件里照样产出形态正确。
