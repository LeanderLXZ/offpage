...
c. **源文件分批规划**：按自然剧情边界切分（默认目标 10 章，最小 5 章，
   ...
   输出：`works/{work_id}/analysis/source_batch_plan.json`
...
- `world/world_event_digest.jsonl` — 从世界 snapshot `stage_events` 自动累积（ID 格式 `E-S###-##`，importance 按关键词推断）
...
3. 关系连续性 — 相邻 batch 间 attitude/trust/intimacy 变化是否有 driving_events
...
9. world_event_digest 对应 — digest 条目数 ↔ world snapshot `stage_events` 逐阶段对应
...
Phase 4 与 Phase 3 数据独立——前置条件仅为 `source_batch_plan.json` 存在
...
阶段号由章节号通过 `source_batch_plan.json` 查得；seq 为该阶段内从 01
起递增的顺序号（上限 99）。
...
- [ ] memory_timeline 条目是否包含 `memory_id`（`M-S###-##`）、`time`、`location`；`event_summary` 是否 ≤ 50 字
...
- 进度追踪：`works/{work_id}/analysis/progress/`（pipeline.json + phase3_batches.json）
