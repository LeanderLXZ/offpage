...
6. memory_digest 摘要一致 — `memory_digest.summary` 与对应 `digest_summary` 文本完全相等（1:1 拷贝契约）
...
10. world_event_digest 摘要一致 — `world_event_digest.summary` 与对应 `stage_events[i]` 文本完全相等（1:1 拷贝契约，i 由 `event_id` 的 seq 推得）
...
报告会以 dirty 状态挡住 `checkout_master`，也会被 squash-merge 漏掉。
...
- [ ] 角色 / 世界 `stage_snapshot` 的 `evidence_refs` 是否有效引用（memory_timeline 不涉及此字段）
- [ ] 世界快照是否与角色快照的阶段对齐
