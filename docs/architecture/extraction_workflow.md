...
- **Recovery sweep**（main ThreadPool 之后、最终 gate 之前触发）：扫
  ...
  的 chunk；用 `[phase0].recovery_effort`（默认 `high`）作 per-call effort 重跑一次
  ...
  `[llm].effort` 跑 + sweep 降到 `high` 救撞墙"是精准兜底而非全局降级；
  相对主流程默认的 `xhigh`（分档判据见决策 #65，键归属见 #69）仍是降档，
  语义成立
...
**target_baseline 的 phase 3 硬约束（双向相等）**：phase 3 stage_snapshot
三结构（`voice_state.target_voice_map` / `behavior_state.target_behavior_map`
/ 顶层 `relationships`）的 keys 必须**双向相等**于
`target_baseline.targets[].target_character_id`——`set(三结构 keys) ==
set(baseline.target_character_ids)`，多/少都 cross-file hard fail。三态由
"内容是否填充"承载（不由 key 是否出现承载）：已登场→正常填、已见过此 stage
...
- `target_baseline.json` — 全书视野下该角色与其它角色之间的全部 target
  ...
  `schemas/character/targets_cap.schema.json` $ref 共享继承
...
3. **target_baseline 的 phase 3 硬约束（双向相等）**：stage_snapshot 三
   结构（`voice_state.target_voice_map` / `behavior_state.target_behavior_map`
