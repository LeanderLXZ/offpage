...
- **Phase 2** baseline 2+2N lane fan-out（lane A `key_figures` 替换先行串行，`fixed_relationships` + 每角色 `identity`+`manifest` / `target_baseline` 两 lane 并行，输入按 lane 投影裁剪；per-lane repair 缩水版——T0/T1 + 程序 checker，无 regen，终点 `validate_baseline` 安全阀，#59；phase 3 三 target 结构与 baseline 双向相等硬门控；重写有 phase 3 产物前置 guard，#56）→ **Phase 3** 逐 stage 1+2N 并行 + char_snapshot 4 sub-lane（`snapshot_merge.py`，#55）+ per-file repair（3 tier T0/T1/T2，按 rule 路由到 `(start,max)` + 每 tier 封顶 2 次，单遍无整文件 regen；`$` 根锚点 issue 永不升 LLM 层（钳到 T0；本就起步 LLM 层的 → `NO_FIX_TIER` 直接 defer）——根补丁交给 LLM = 全文重写，即被删的 T3；T1/T2 patch 后均即时 scoped 复验才算 resolved；scoped L3 gate 逐文件只复检「本轮改过的 json_path ∪ 本轮携带的未修语义 issue path」（后者不可省，否则未修 issue 被 round-diff 判 resolved = 假 PASS）、代码层过滤返回值、后端失败类 issue 永不过滤，#62/#66；本轮零 patch 的文件不进 gate，其未修语义 issue 原样携带进 blocking 集，#67）+ PP 重跑 + commit SHA 非空才 `COMMITTED`（`defer_unresolved_semantic` 开时未决可延后类别 semantic/schema/structural/cross_file + coverage_shortage 残留写 `deferred_repairs/` 台账后当 PASS 继续，逐文件判定，仅 json_syntax / worker 崩溃硬停机，#60）→ **Phase 3.5** 最终关卡，六段串行（段内并行）：①程序全扫 → ②结清台账债 → ③跨阶段连贯审校 → ④审校发现定点修 → ⑤受影响 stage 重投影 → ⑥复扫门判（#72）。三层检查：L1 结构在位 / L2 派生 1:1（#61 后唯一能发现投影漂移处）/ L3 台账结清（schema 类重校验自愈、走与修复循环同一个长度容差判据；semantic 与 unverified 类凭 resolution 记录，unverified 按 rule 判定、那条记录由段 2/4 重跑审校产出，#74）。段 3 是全流水线唯一多阶段并列视图，吃程序拼的瘦投影（`phases/cross_stage_projection.py`，按 `[phase3_5].review_window_chars` 切窗、重叠 1 stage），只定位不修；发现先做存在性确认，`$` 根锚点丢弃。段 2/4 复用 `repair.run(seed_issues=...)` 跳过发现扫描、定点 patch 零重生成；每文件两次机会（第二次从残留 re-seed + 改用 `[llm].effort`，审校器失败与抛异常都不计入），未结清的回写台账使「哪个段发现的」不再影响裁决方式（无 stage 可挂的发现例外，直接并入判定），两次仍失败写 `given_up` 降为 warning 放行并单独成段列出（#74）。门判 `error==0 && skipped==0`——coverage 账本让"没检查"不再伪装成"检查通过"。→ **Phase 4** 场景归档（独立，仅需 `stage_plan.json`；`--start-phase 4`）。
- 关键设计：lane 级 resume + 启动磁盘 reconcile 自愈；`RateLimitController` token 限额自动暂停（`docs/requirements.md` §11.13）；长度容差门控最终安全阀（#48）；`--resume` phase 无关续跑 + `--background` 校验（#51/#56）；配置单源 `extraction/config.toml`（CLI > local > toml > 默认）；run 指标账本 `core/run_metrics.py`（`run_with_retry` 单点每调用落 `logs/runs/{work_id}_{ts}.jsonl` 时间/token/cost，`set_phase` 标注 phase，`call_type` 由 repair 调用点标注检查/修复类型（#71），run 末按 phase/lane/call_type 聚合表；best-effort no-op）。
