...
- **Phase 1** — global analysis (identity merge → world overview → stage plan → candidates). Three outputs (`world_overview.json` / `stage_plan.json` / `candidate_characters.json`) all gated by jsonschema (`schemas/analysis/{world_overview,stage_plan,candidate_characters}.schema.json`); schema fails + stage chapter-count violations (monolithic 5–15) merge into `correction_feedback` and share `[phase1].exit_validation_max_retry` budget. **Dual-mode dispatch**: `monolithic` runs LLM stage-boundary discovery against the 5–15 chapter rule; `light_novel` derives `stage_plan` 1:1 from `chapter_index` programmatically (no LLM call for stage-boundary), with `stage_id = S{n:03d}`, `chapters = f"{chapter_id}-{chapter_id}"` (degenerate single-chapter range so phase 2/3/4 parsers work unchanged), `chapter_count = 1`, `stage_title = chapter_index[i].title` (soft-truncated to the schema's `stage_title.maxLength` cap + `…` if the derived value would exceed it); STAGE_MIN / STAGE_MAX validation bypassed under `light_novel`. Phase 2+ does not branch on `structure_mode` — `stage_plan` is the single contract downstream.
...
- **CLI `--resume` phase-agnostic resume** — `run_full` is the sole resume entry point: per-phase skip-detection (Phase 0 schema-gated chunk skip / Phase 1 stage_plan + candidate_characters product check / Phase 1.5 `--characters` bypass / Phase 3 `reconcile_with_disk` + rebuild `phase3_stages.json` from `stage_plan.json`). `--resume` only silences the `'Resume from existing progress?'` prompt inside `run_full`. `--background` validation is stage-aware via `pipeline.json::phases.phase_1_5`: not done → require `--characters` (else daemon hits `confirm_with_user` stdin); done → require `--resume` or `--characters` (else daemon hits the resume prompt). → `automation/persona_extraction/cli.py` + `orchestrator.py::run_full(auto_resume=...)` + decision #51.
