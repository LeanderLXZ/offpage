...
- **Phase 2** — baseline production (world foundation + per-character `identity.json` + per-character `target_baseline.json`, draft). `target_baseline.json` is the full-book-view roster of every target character (with `tier` + `relationship_type` + ≤100-char description) the subject ever interacts with; phase 3 stage_snapshot target keys must be ⊆ `targets[].target_character_id` (cross-file hard fail; baseline immutable from phase 3 onward).
