...
27m. **stage_plan 切分语义 = 拐点先行，章数硬范围；prompt 反锚定 +
     ...
     stage（章数 8-15 hard，schema `chapter_count.maximum=15` 强制上限 +
     orchestrator `_check_stage_plan_limits` 兜底 8-15 monolithic 双向；
     schema `chapter_count.minimum=1` 是为 light_novel 1:1 派生让出空间，
     monolithic 下限 8 由 `_check_stage_plan_limits` 强制）；2.3 反锚定
     自检（≥3 连等章数视为机械等分必须重审 +
