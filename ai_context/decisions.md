...
26. Extraction runs on `extraction/{work_id}` branch. Each passing stage committed. Rollback = `git reset`. **Squash-merge to `library` on completion** (never to `master`). Three-branch model: `master` = framework only, pushed to remote; `extraction/{work_id}` = per-work in-progress, local; `library` = completed-works archive, local. `library` absorbs framework updates via periodic `git merge master`; nothing flows back to master, keeping the public-facing branch artefact-free. Squash target controlled by `[git].squash_merge_target` (default `library`).
