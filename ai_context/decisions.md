...
27j. **Phase 0/1 dual-mode dispatch via `structure_mode`.** Source manifest
     ...
     manifest is the single source of truth — `extraction/ingestion/
...
27m. **Chunk-level secondary fields on `chapter_summary_chunk`.** Phase 0
     ...
     **double-pipe to `foundation.major_factions.key_figures`**:
     ...
     `extraction/persona_extraction/prompts/{summarization,analysis_foundation,baseline_production}.md`,
...
57. **Extraction 包结构 — 顶层 `extraction/` + 内部分 `persona_extraction` /
    `validation` / `ingestion` / `repair` 四子包 + validation 框架预留位。**
    `extraction/` 是所有数据抽取相关代码（orchestrator + 相位特化 + 数据正确性 +
    pre-Phase-0 ingestion gate）的唯一顶层 package。`persona_extraction/`
    内按职责分 4 子目录：`core/`（config / schema_loader / llm_backend /
    rate_limit / git_utils / process_guard / json_repair — 7 个框架无关
    基础件）+ `lifecycle/`（progress / manifests / lane_output /
    failed_lane_log — 4 个跨相位状态机）+ `phases/`（scene_archive /
    snapshot_merge / post_processing — 3 个相位产物生成）+ `prompts/`
    （9 个 .md 模板，由 `prompt_builder.py` 在 `Path(__file__).parent /
    "prompts"` 加载）+ `tests/`（6 个 `_smoke_*.py`，运行入口
    `python -m extraction.persona_extraction.tests._smoke_X`）；package 根仅
    `__init__.py / __main__.py / cli.py / orchestrator.py / prompt_builder.py`
    5 个文件。`validation/` 顶层独立子包，分 `gates/`（`phase2_baseline.py` /
    `phase3_5_consistency.py`——orchestrator 在相位边界调用、返回
    `ValidationReport`、不触发 fixer 循环）+ `shared/`（`importance.py` /
    `schema_tolerance.py`——纯函数原语，gates 与 `extraction.repair.checkers/*`
    都直接 import）+ `types.py`（`ValidationIssue` dataclass） + `README.md`
    （声明：未来 `T-PHASE2-REPAIR-AGENT` 拍板后 gates 内文件会重构为
    `BaseChecker` 子类并入 `repair/checkers/`，shared 与 types 永久保留）。
    `repair/`（已接入框架的 L0–L3 checker × T0–T3 fixer 矩阵）与 `validation/`
    并列：repair 是已成体系的 framework 内住户、validation 是 framework 外
    pass/fail gate + 共享原语，两者**正交且依赖单向**（repair/checkers/*
    可以 `from extraction.validation.shared.*`，validation/* **绝不** import
    `extraction.repair.*`）。`ingestion/` 守 pre-Phase-0 source 包 schema 校验，
    位置不动。**依赖方向硬约束**：(1) repair/checkers → validation.shared 单向；
    (2) repair/* 不进 persona_extraction.phases.*；(3) validation/* 不进
    extraction.repair.*；(4) **validation.gates 可依赖 `extraction.persona_extraction.core`
    提供的纯函数基础件**（`json_repair` / `schema_loader`）——gates 是面向具体相位的
    validator 实现，需要项目内通用 IO helpers；**validation.shared 严格 zero-dep**，
    只用 stdlib + `jsonschema`。**显式不做**：(a) 不切 `orchestrator.py`（3744 行，
    正交大工程）；(b) 不把 `gates/` 内文件重构成 `BaseChecker` 子类（等
    `T-PHASE2-REPAIR-AGENT` 拍板）；(c) 不动 `ingestion/validator.py` 位置
    （作用域不同——验 sources/ 而非 canon）；(d) 不动 repair/checkers + fixers
    内部结构（已是好榜样）；(e) `Config.repair` 属性与 `[repair]` TOML section 同名；
    `RepairAgentConfig` dataclass 类名保留，以避开与 `extraction.repair.protocol.RepairConfig`
    （per-run config）的命名冲突——属性名 / section 名 / dict key 三方一致，
    类名因冲突独立。
    Plumbing → `extraction/` 全包 + `extraction/persona_extraction/{core,lifecycle,
    phases,tests}/__init__.py` + `extraction/validation/{__init__.py,
    types.py,README.md,gates/__init__.py,shared/__init__.py}` + `extraction/
    repair/tests/__init__.py` + `extraction/config.toml`（`[repair]` section
    + 注释路径）+ `extraction/pyproject.toml`（description）+ `.gitignore`
    （`extraction/config.local.toml`）+ `.claude/hooks/session_branch_check.sh`
    （pgrep 模式）+ `ai_context/skills_config.md`（`## Source directories`）+
    `ai_context/conventions.md`（Cross-File Alignment 表新增本条对应行：
    `extraction/` 包内文件迁移触发清单）+ `extraction/README.md` + `docs/
...
44. `prompts/` = manual scenarios only (ingest / review / supplement / cold start). Extraction prompts in `extraction/persona_extraction/prompts/`; runtime rules in `simulation/prompt_templates/`. Self-contained modules.
