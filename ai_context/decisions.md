...
27m. **stage_plan 切分语义 = 拐点先行，章数硬范围；prompt 反锚定 +
     ...
     `extraction/persona_extraction/{orchestrator,progress}.py`、
...
55. **char_snapshot lane 拆 4 sub-lane 并行 + prev snapshot 按 lane 切片喂入
    ...
    **Fallback** `false` → 单 lane 等价 `lane_scope=ALL`，phase 3 现状不变
    ...
    `extraction/persona_extraction/{snapshot_merge,prompt_builder,orchestrator,progress,config,cli,lane_output}.py`、
...
57. **Extraction 包结构 — 顶层 `extraction/` + 内部分 `persona_extraction` /
    ...
    extraction.repair.*。**显式不做**：(a) 不切 `orchestrator.py`（3744 行，
    ...
    内部结构（已是好榜样）；(e) Config dataclass 类名 `RepairAgentConfig` 保留
    以避开与 `repair.protocol.RepairConfig`（per-run config）的命名冲突——
    仅属性 `repair_agent` → `repair` + TOML section `[repair_agent]` → `[repair]`。
    Plumbing → `extraction/` 全包 + `extraction/persona_extraction/{core,lifecycle,
    phases,prompts,tests}/__init__.py` + `extraction/validation/{__init__.py,
