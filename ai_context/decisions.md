...
27m. **Chunk-level secondary fields on `chapter_summary_chunk`.** Phase 0
     ...
     **Phase 1 foundation lane mapping** (decision #52 + #54):
     ...
     `chunk_factions → major_factions` (description only) /
     ...
     phase 2 只在 baseline 阶段单独 LLM call 补齐
     `foundation.major_factions.key_figures` (decision #54). Explicitly
     **NOT added**: `chunk_fixed_relationships[]` (chunk view ≤25
     ...
     — `key_figures` 由 phase 2 baseline 单独 LLM call 补齐（输入：phase 1
     落盘的 foundation.json + candidate_characters.json + 已确认目标清单）
     映射 raw 名 → `character_id`；`members_present` 仍由 phase 1
     candidate_characters lane 用做跨 chunk 身份合并。
