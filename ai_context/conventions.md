...
| `deferred_repair_log.DEFERRABLE_CATEGORIES`（可延后类别集） | `phase3_5_consistency._REVALIDATABLE_CATEGORIES` —— 新增类别须同时决定 L3 如何结清它（程序复验 vs 凭 resolution）；两侧常量名不同，grep 查不到对方 |
