...
- **Bounds only in schema.** All `maxLength` / `minLength` / `maxItems` / `required` live in `schemas/**.schema.json`; no duplicates anywhere else. Exact values → schema file. Index → `docs/architecture/schema_reference.md`. Cross-schema sharing of a single bound (e.g. target-array cap shared by `target_baseline.targets` + stage_snapshot's three target maps) is done via `$ref` to a shared fragment under `schemas/_shared/` — still single-source, no duplication.
