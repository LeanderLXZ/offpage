...
| `extraction/` 包内文件迁移（在 `persona_extraction/{core,lifecycle,phases,prompts,tests}/` / `validation/{gates,shared}/` / `repair/{checkers,fixers,tests}/` 之间挪文件，或子包改名） | 所有 `from .xxx` / `from ..xxx` 相对 import 同步加深 / 减层；所有 `from extraction.X.Y` 绝对 import 同步；`extraction/config.toml` 注释路径 + section 名（如 `[repair]`）+ `extraction.persona_extraction.core.config.load_config()` 入口提示；`ai_context/skills_config.md ## Source directories` 列表项；`.claude/hooks/session_branch_check.sh` 第 24 行 `pgrep -f 'extraction\.persona_extraction'` 模式；`docs/architecture/{extraction_workflow,schema_reference,data_model}.md` + `ai_context/{architecture,decisions,conventions,handoff}.md` + `extraction/README.md` 内所有路径引用；`schemas/**/*.schema.json` 描述里若 `$comment` 引用代码路径同步；`prompts/{ingestion,review}/*.md` 内若引用代码路径同步。决策 #57。 |
