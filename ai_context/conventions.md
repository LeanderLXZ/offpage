...
- **Bounds only in schema.** All `maxLength` / `minLength` / `maxItems` / `required` live in `schemas/**.schema.json`; no duplicates anywhere else. Exact values → schema file. Index → `docs/architecture/schema_reference.md`. Cross-schema sharing of a single bound is done via `$ref` to a shared fragment located near the schemas it serves: cross-domain shares live under `schemas/_shared/`; single-domain shares live in that domain's directory (e.g. target-array cap is shared by `target_baseline.targets` + stage_snapshot's three target structures — both files live in `schemas/character/`, so the fragment lives there as `schemas/character/targets_cap.schema.json`). Still single-source, no duplication.
