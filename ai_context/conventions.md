...
- Length gates (hard): declared in JSON schema only (`schemas/**.schema.json`) — no parallel tunables in `config.toml`. Spot examples: `stage_events` items 50–80 chars, `memory_timeline.event_description` 150–200, `memory_timeline.digest_summary` 30–50, `memory_timeline.subjective_experience` 100–200, `relationship_history_summary` ≤ 300, `knowledge_scope` items ≤ 50, character baseline string fields (`identity` / `voice_rules` / `behavior_rules` / `boundaries` / `failure_modes`) typically 50 / 100 / 200; `voice_rules.signature_phrases` items ≤ 10 chars, `voice_rules.typical_expressions` items ≤ 15 chars (tighter because they are verbatim phrases).
- Count caps (hard): schema `maxItems`. Spot examples: `knowledge_scope.knows` ≤ 50, `does_not_know` ≤ 30, `uncertain` ≤ 30, `memory_timeline.knowledge_gained / misunderstanding / concealment` ≤ 5 each, `behavior_rules.core_goals / obsessions` ≤ 10, `behavior_rules.emotional_reaction_map` ≤ 15, `identity.core_wounds` ≤ 15, `identity.key_relationships` ≤ 10, `identity.distinguishing_features` ≤ 20, `voice_rules.emotional_voice_map` ≤ 15, `voice_rules.target_voice_map` ≤ 10, `voice_rules.signature_phrases` ≤ 30, `boundaries.*` ≤ 15, `failure_modes.*` ≤ 10–15. Over-limit → trim least relevant.
- Baseline `evidence_refs` / `source_type` removed: `identity` / `voice_rules` / `behavior_rules` / `boundaries` / `failure_modes` / `memory_timeline_entry` carry no chapter-level trace fields. Chapter anchors live on `stage_snapshot` (`evidence_refs`) and `world_stage_snapshot` (`evidence_refs`); `memory_timeline` uses `scene_refs`.
