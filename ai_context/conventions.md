...
| `schemas/character/stage_snapshot.schema.json` 顶层属性增删 / 重命名（含 `stage_delta` / `failure_modes` 子键） | sub-lane 字段归属表 `automation/persona_extraction/snapshot_merge.py::FIELD_ALLOCATION`（同源给 prompt + merge 用，新增 / 改名属性必须挂到对应 sub-lane，否则 merge hard gate 报"字段集合不全覆盖"）+ `automation/prompt_templates/character_snapshot_extraction.md`（`{lane_scope}` 注入的"本次仅写字段"白名单段）+ `docs/architecture/extraction_workflow.md` §6.2 sub-lane 字段表 + `ai_context/decisions.md` #55 + `docs/requirements.md` §9.3。决策 #55。 |
