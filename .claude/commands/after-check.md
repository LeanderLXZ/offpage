...
这不是全仓 review（那是 `/check`），只针对这次 `/go` 触及的细节；也不是二次 finding 挖掘，不新增与本次改动无关的问题。`$ARGUMENTS` 存在则作为本轮聚焦方向或额外关注点。
...
- 只读不写：不改代码、不改文档、不落盘报告（归档是 `/check` 的事）
- 每条 finding 必须落到文件 + 行号；直接证据与推断分开标注
- 不要因为 `/go` Step 5 已经 review 过就走过场 —— 此轮用新的眼睛再看一遍，重点抓 `/go` 漏掉的连带文件和歧义

---

**镜像约束**：本文件和 `.agents/skills/after-check/SKILL.md` 正文保持同步——任一侧修改必须在同 commit 内镜像到另一侧。`.agents/skills/after-check/SKILL.md` 额外带 YAML frontmatter（`name` / `description`），正文（从一级标题 `# /after-check` 起往下）与本文件**逐字一致**。
