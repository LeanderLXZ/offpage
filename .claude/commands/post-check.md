...
下方流程分为 `## Step 0:` ~ `## Step 7:`（含子 step `## Step 1.5:`）。每进入一个 step，**先打印一行进度**：`[/post-check] Step N: <子段标题>`，让用户实时看到当前位置；最后一步执行结束打印 `[/post-check] done`。跳过某 step 时打印 `[/post-check] Step N: 跳过（理由：…）` 而不是静默略过。
