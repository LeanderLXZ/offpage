...
| `character/` | 角色 baseline + 阶段目录 + 阶段快照 + 记忆 | `identity`、`character_manifest`、`stage_catalog`、`stage_snapshot`（含内联 `failure_modes` / `voice_state` / `behavior_state` / `boundary_state` 全字段）、`memory_timeline_entry`、`memory_digest_entry` |
