...
16. `active_aliases` 是否存在？primary_name 是否正确？active_names 是否覆盖本阶段出场名称？known_as 是否覆盖主要关系角色？
17. `voice_state` 是否有足够的 emotional_voice_map（至少 3 种情绪）？dialogue_examples 是否有 2-3 条？target_voice_map 每个 target 是否有至少 3-5 条 dialogue_examples？
18. `behavior_state` 是否有足够的 emotional_reaction_map（至少 3 种情绪）？是否有 target_behavior_map？每个 target 是否有至少 3-5 条 action_examples？
19. `boundary_state` 是否存在？软边界是否合理？
20. `relationships` 是否覆盖了本阶段出现的所有重要角色？每条是否有 driving_events？
21. `knowledge_scope` 是否存在？knows / does_not_know / uncertain 是否合理？
22. `misunderstandings` 和 `concealments` 是否已填写（即使为空数组）？
23. `emotional_baseline`、`current_personality`、`current_mood`、`current_status` 是否存在？
24. `stage_delta` 是否存在（非首阶段时应有实质内容）？
25. `memory_timeline` 是否覆盖了本阶段的关键事件？
26. `memory_digest.jsonl` 是否包含本阶段所有 memory_timeline 条目的对应摘要？
27. 有没有明显的重大遗漏？

### C2. 别名一致性

28. identity.json 的 aliases 是否包含本阶段新发现的别名（双写检查）？
29. stage_snapshot 的 active_aliases 与 identity.json 的 aliases 是否一致（本阶段活跃的别名都应在 identity.json 中有对应条目）？

### D. 世界-角色交叉一致性

30. 世界快照和角色快照使用的 stage_id 是否一致？
31. 世界快照中的重大事件是否在相关角色的 stage_events（仅本阶段）中有体现？
32. 角色 relationships 中引用的事件是否在世界事件中有对应？
