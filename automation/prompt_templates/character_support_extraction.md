...
- 每条 `event_description` 写到该事件实际信息量需要的长度即止；上下限范围
  是门控，不是"应该写到上限"。
- identity 修正只在原文确实推翻或补充骨架时做；没有改动就不动该字段，
  不要为了"显示有修正"而强行改写。
...
1. **仅产出 memory_timeline + identity 修正**：本次调用负责 `{character_id}` 的 memory_timeline 和（按需）identity.json 修正。stage_snapshot 由独立调用完成
2. **独立于快照**：memory_timeline 是逐事件的主观记忆录制，独立于 stage_snapshot 的聚合状态。你不需要参照快照内容
3. **identity 修正**：identity.json 在 Phase 2 全书分析阶段产出骨架初稿。任何阶段中如发现原文与初稿不符（aliases 漏录、core_wounds / key_relationships 描述不准等），应直接修正。注意：identity 只记录跨阶段稳定的角色基础事实（aliases / core_wounds / key_relationships 等），阶段性变化（语气、行为、边界、崩坏防护）写入 stage_snapshot（由独立调用处理），**不**写入 identity
...
**不要写入 stage_snapshot**。stage_snapshot 由独立调用处理（含本阶段全量 voice / behavior / boundary / failure_modes 字段，不属于 char_support 范围）。
...
- identity 修正过于激进（阶段性变化应归 stage_snapshot，不应写入 identity）
...
| 文件 | 错误 | 正确（schema 权威） |
|------|-----|-------------------|
| identity.json | 顶层添加 `description` 字段 | **不要添加**——schema 设置了 `additionalProperties: false` |
