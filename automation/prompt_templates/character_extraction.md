...
3. **Baseline 修正**：所有 baseline 文件（identity.json、voice_rules.json、behavior_rules.json、boundaries.json、failure_modes.json）及 world foundation 已在 Phase 2.5 全书分析阶段产出初稿（标注 source_type: inference）。任何阶段中如发现原文与初稿不符，应直接修正（把 inference 升级为 canon）。注意：这些 baseline 只记录跨阶段稳定的角色基底，阶段性变化写入 stage_snapshot
...
**任何阶段可修正和补充的 baseline**（读到原文细节后把 inference 升级为 canon）：
