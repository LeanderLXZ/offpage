...
**每批产出**：
- `characters/{character_id}/canon/stage_catalog.json` — 追加本阶段条目
...
- `characters/{character_id}/canon/memory_digest.jsonl` — **追加**本阶段的压缩摘要条目（遵循 memory_digest_entry.schema.json，见下方说明）
...
每个角色的 `memory_digest.jsonl` 是 memory_timeline 的压缩索引，运行时全量加载。
...
- `location`：原样保留（可选）
- `emotional_tags`：从 `emotional_impact` 提取 2-4 个关键情绪词（可选）
- `involved_targets`：从 `relationship_impact` 提取涉及的角色名列表（可选）

**控制目标**：每条 digest ~60-80 tokens。

## 本批输出清单

完成后，请确认已产出以下内容：

1. `{character_id}` 的 stage_snapshot + memory_timeline + memory_digest + stage_catalog 更新
2. 所有文件都通过 schema 校验
3. evidence_refs 非空
4. source_type 已标注
{retry_note}
