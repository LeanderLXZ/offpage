...
必须遵循 `schemas/fixed_relationships.schema.json`。
...
**注意**：后续提取追加 stage 条目时，每条**必须**包含 `stage_id`、`order`（整数序号）、`title`、`summary`（一句话摘要）、`snapshot_path`（快照相对路径）。stage_catalog 仅用于 bootstrap 阶段选择，运行时不加载。

## 不在 baseline 阶段产出的文件

以下文件由 `post_processing.py` 在每批提取完成后程序化维护（0 token），
baseline 阶段**不需要**创建任何占位文件：

- `{work_dir}/world/world_event_digest.jsonl` — 从世界 stage_snapshot 的
  `stage_events` 生成；首批提取后首次创建，后续批次 upsert。
- `{work_dir}/characters/{{char_id}}/canon/memory_digest.jsonl` — 从角色
  memory_timeline 生成；同样由首批 post-processing 首次创建。
