...
- `{work_dir}/characters/{{char_id}}/canon/stage_catalog.json`（遵循 `schemas/work/stage_catalog.schema.json`）
...
**注意**：后续提取追加 stage 条目时，每条**必须**包含 `stage_id`（紧凑英文代号 S###）、`order`（整数序号）、`stage_title`（≤15 字短标题）、`summary`（一句话摘要）、`snapshot_path`（快照相对路径）。stage_catalog 仅用于 bootstrap 阶段选择，运行时不加载。
...
- 中文作品的 work_id、character_id、路径段、内容使用中文；`stage_id` 使用紧凑英文代号 `S###`（如 `S001`），`stage_title` 使用 ≤15 字中文短标题
