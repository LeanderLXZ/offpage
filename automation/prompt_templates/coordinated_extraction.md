...
- **memory_id**：必须使用统一格式 `M-S{stage:03d}-{seq:02d}`（例：`M-S001-01`）。3 位阶段号支持 ≤ 999 阶段，2 位 seq 每阶段从 01 递增（上限 99）。`time` 字段统一命名（旧 `time_in_story` 已废弃）
...
- `memory_id`：复制自 memory_timeline（阶段号已编码在 ID 中，无需冗余 stage_id）
...
**已移除的旧字段**：`stage_id`（改由 memory_id 前缀 `S###` 推导）、`emotional_tags`、`involved_targets`（降噪，需要细节时走 FTS5）。
