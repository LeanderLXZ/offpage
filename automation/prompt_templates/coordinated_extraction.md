...
3. **Baseline 修正**：所有 baseline 文件（identity.json、voice_rules.json、behavior_rules.json、boundaries.json、failure_modes.json）及 world foundation 已在 Phase 2.5 全书分析阶段产出初稿（标注 source_type: inference）。任何阶段中如发现原文与初稿不符，应直接修正。Baseline 记录跨阶段稳定的角色基底，阶段性变化写入 stage_snapshot
...
- **time**：故事内时间
- **event_summary**：客观描述应包含关键要素（谁、做了什么、在什么情境下），**必须 ≤ 50 字**——这是 memory_digest 的直接来源，提取阶段负责字数控制。详细经过写在 `subjective_experience`
- **subjective_experience**（最关键字段）：用角色第一人称视角深入展开——角色当时在想什么、感受到什么、为什么会这样感受、这件事与角色过去经历的联系、角色内心的矛盾或挣扎。至少 3-5 句，重要事件可以更长（不受 50 字限制）。不要写空泛的形容词堆砌，要写出具体的心理过程
...
**正面示例**（event_summary ≤ 50 字；subjective_experience 有细节、有心理过程）：
```json
{
  "memory_id": "M-S003-02",
  "time": "三年后",
  "event_summary": "她在某处遇见本以为已死去三年的角色A，对方气质大改",
...
- `summary`：复制自 `event_summary`，**≤ 50 字**（由 memory_timeline 侧在提取时完成字数控制）
...
**LLM 职责**：只需保证 `memory_timeline/{stage_id}.json` 的 `event_summary ≤ 50 字`、`memory_id` 符合 `M-S{stage:03d}-{seq:02d}` 格式，digest 生成完全由脚本完成。
