...
```json
{
  "event_summary": "角色B在某处遇到了已经'死去'三年的角色A，对方容貌未变但气质大改，周身弥漫着陌生的气息",
...
每个角色的 `memory_digest.jsonl` 是 memory_timeline 的压缩索引，运行时 stage 1..N 过滤加载，让 LLM 感知历史记忆并判断是否需要 FTS5 检索详情。
