...
- `identity_notes`: 如果本章中出现角色身份相关的线索（角色获得新名称、揭示真实身份、开始使用化名、被赋予称号等），在此记录。例如："主角得知'寒剑'的真名为'寒淼'"、"王枫化名'XX'潜入敌营"。如无此类事件则留空字符串
...
- `characters_present` 只记录有实质互动的角色，不记背景群演。**注意角色可能以化名、代称、昵称等出现**——如果你能判断某个名称实际上是已知角色的别名，在 `characters_present` 中使用其最常用的名称，并在 `key_events` 中注明化名关系（如"王枫以XX身份潜入"）
