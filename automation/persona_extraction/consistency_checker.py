...
def _check_evidence_refs_coverage(
    ...
    """Flag snapshots with empty evidence_refs and memory entries with
    empty ``scene_refs``.
    ...
    For ``memory_timeline`` entries we only check ``scene_refs`` — the
    scene back-pointer is how runtime historical recall resolves a
    ...
    for char_id in character_ids:
        for stage_id in stage_ids:
            ...
            # Check memory_timeline entries — scene_refs must be
            # non-empty for full historical recall traceability.
