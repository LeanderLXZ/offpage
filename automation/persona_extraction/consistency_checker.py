...
def _check_evidence_refs_coverage(
    ...
    For ``memory_timeline`` entries, also flags empty ``scene_refs`` —
    the scene back-pointer is how runtime historical recall resolves a
    memory to its originating ``scene_archive`` entry. Empty here means
    the retrieval path degrades silently. Existence-only check for now:
    full ``scene_id`` resolution against ``scene_archive.jsonl`` is out
    of scope (scene_archive is local-only and generated by Phase 4).

    World stage_snapshot ``evidence_refs`` is checked symmetrically —
    the field is defined by ``world_stage_snapshot.schema.json``, and
    an empty list means the world-level snapshot has no chapter anchor
    ...
    for char_id in character_ids:
        for stage_id in stage_ids:
            ...
            # Check memory_timeline entries — both evidence_refs (source
            # chapter anchor) and scene_refs (scene_archive back-pointer)
            # must be non-empty for full historical recall traceability.
            ...
            if isinstance(timeline, list):
                for i, entry in enumerate(timeline):
                    ...
                    if not entry.get("scene_refs"):
                        issues.append(ConsistencyIssue(
                            "warning", "scene_refs",
                            f"{char_id}/{stage_id}/memory/{mid}",
                            "memory_timeline entry has empty scene_refs"))
