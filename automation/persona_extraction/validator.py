...
def _check_world(work_dir: Path, stage_id: str,
    ...
    if not snapshot_path.exists():
        ...
        if snapshot is not None:
            ...
            # stage_events char limit (≤ 80 chars per entry)
            for i, ev in enumerate(snapshot.get("stage_events") or []):
                if isinstance(ev, str) and len(ev) > _STAGE_EVENT_MAX:
                    issues.append(ValidationIssue(
                        "warning", str(snapshot_path),
                        f"stage_events[{i}] is {len(ev)} chars "
                        f"(want <= {_STAGE_EVENT_MAX})"))
...
def _check_world_event_digest(path: Path,
    ...
    for i, line in enumerate(text.splitlines()):
        ...
        if len(summary) > _EVENT_SUMMARY_MAX:
            issues.append(ValidationIssue(
                "warning", f"{path}:line[{i}]",
                f"summary is {len(summary)} chars "
                f"(want <= {_EVENT_SUMMARY_MAX})"))
...
def _check_memory_timeline(path: Path,
    ...
    for i, entry in enumerate(data):
        ...
        evt_summary = entry.get("event_summary") or ""
        if len(evt_summary) > _MEMORY_SUMMARY_MAX:
            issues.append(ValidationIssue(
                "warning", label,
                f"event_summary is {len(evt_summary)} chars "
                f"(want <= {_MEMORY_SUMMARY_MAX})"))
...
def _check_memory_digest(path: Path, stage_id: str,
    ...
    for i, line in enumerate(text.splitlines()):
        ...
        summary = entry.get("summary") or ""
        if len(summary) > _MEMORY_SUMMARY_MAX:
            issues.append(ValidationIssue(
                "warning", f"{path}:line[{i}]",
                f"summary is {len(summary)} chars "
                f"(want <= {_MEMORY_SUMMARY_MAX})"))
