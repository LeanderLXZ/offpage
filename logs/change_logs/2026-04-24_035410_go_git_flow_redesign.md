...
- **Branch**: master worktree（`/home/leander/Leander/persona-engine-master`），主 checkout 仍在 `extraction/我和女帝的九世孽缘`
- **Status**: PRE
...
- Step 0：当前在 `extraction/我和女帝的九世孽缘`（非 master） → 自动 `git worktree add /home/leander/Leander/persona-engine-master master`（打印策略：`../persona-engine-master worktree 隔离`）
