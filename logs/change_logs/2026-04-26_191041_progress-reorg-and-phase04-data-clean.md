...
- **Branch**: main（worktree `../offpage-main`，主 checkout 留在 `extraction/<work_id>`，clean）
- **Status**: DONE
...
| 路径 | 类型 | 处理 |
|---|---|---|
| `works/<work_id>/analysis/chapter_summaries/` (22 chunks) | Phase 0 LLM 产物 (gitignored) | `rm -rf` |
| `works/<work_id>/analysis/scene_splits/` (537 files) | Phase 4 中间产物 (gitignored) | `rm -rf` |
| `works/<work_id>/retrieval/scene_archive.jsonl` (1236 行 / 4MB) | Phase 4 最终产物 (gitignored) | `rm -f` |
| `works/<work_id>/analysis/progress/phase0_summaries.json` | Phase 0 state | `rm -f` |
| `works/<work_id>/analysis/progress/phase4_scenes.json` | Phase 4 state | `rm -f` |
| `works/<work_id>/analysis/progress/extraction.log{,.1}` | runtime 日志 | `rm -f`（fresh start） |
| `works/<work_id>/analysis/progress/repair_*.jsonl` (22 个) | Phase 3 残留（其 target 文件已在前轮 /go 删除，记录已 stale） | `rm -f` |
| `works/<work_id>/analysis/progress/pipeline.json` | 标 phase_0/1/2/4 done、phase_3 running、phase_3_5 pending；与已删数据矛盾 | `rm -f`，让 orchestrator next run 重建 |

**保留**：
- `works/<work_id>/analysis/progress/phase3_stages.json`（Phase 3 lane state，重抽 Phase 3 时由 reconcile_with_disk 重置；不阻塞）
- `works/<work_id>/analysis/progress/failed_lanes/`（历史失败 lane 日志，留作诊断）
- `works/<work_id>/analysis/progress/rate_limit_pause.json.lock`（rate limit 状态文件，无害）
...
- [ ] 数据清盘后磁盘验证：`ls works/<work_id>/analysis/chapter_summaries/ 2>&1` 报 No such file；`ls retrieval/` 不含 scene_archive.jsonl；`ls progress/repair_*.jsonl 2>&1` 无匹配
