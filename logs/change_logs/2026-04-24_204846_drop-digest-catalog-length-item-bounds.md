...
- **Branch**: master（worktree `../persona-engine-master`，原仓 dirty 并停留在 `extraction/<work_id>`）
- **Status**: PRE
...
- [ ] 用修改后的 schema 校验现有 `works/<work_id>/` 下 4 份产出文件，仍然 pass（更宽松的 schema 不应让原本合规的数据失败）
...
- [x] 用修改后 schema 校验现有 `works/<work_id>/` 数据：相比 master HEAD schema 错误数从 48 降至 27（更宽松，未引入新错误）；残留 27 条全部是 `location` required / `order` 多余字段（来自更早的 commit cef6cad / §27e 演进，与本次无关）
