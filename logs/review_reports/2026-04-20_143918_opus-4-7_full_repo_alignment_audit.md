...
- 可复现性：对任何不是"<work_id>"的新作品，schema 里的例子都是
  misleading 甚至误导。
...
committed"的 canonical 产物，`works/<work_id>/indexes/` 目录却
...
- 仓库实际状态：`ls works/<work_id>/indexes/` → `No such file`。
...
**位置**：[works/<work_id>/analysis/progress/pipeline.json](works/<work_id>/analysis/progress/pipeline.json)
...
| `docs/` / `ai_context/` vs `works/<work_id>/` 样例 | **中低** | M2（consistency_checker）、M4（sample 缺 lane_states）、M2（indexes 缺失）三条叠加 |
...
> - 不要动 `works/<work_id>/` 下任何产出文件（样例是真实 extraction
>   结果，修动会污染数据）。
